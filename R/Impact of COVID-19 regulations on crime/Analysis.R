...
# Set paths
dir        <- "" # change to suitable repository to access data and save results
dirData    <- paste0(dir, "/_data/")
dirResults <- paste0(dir, "/_results/")
setwd(dir)
# ------------------------------------------------------------------------------
...
# Test for time-fixed effects - Lagrange multiplier test
# Null hypothesis is that no time-fixed effects are needed
# If p < 0.05 use time-fixed effects
  rbind(
    as.numeric(plmtest(FETotal, c("time"), type = ("bp"))[2]), # Use
    as.numeric(plmtest(FETheft, c("time"), type = ("bp"))[2]), # Use
    as.numeric(plmtest(FEHom,   c("time"), type = ("bp"))[2]), # Don't Use
    as.numeric(plmtest(FECyber, c("time"), type = ("bp"))[2]), # Use
    as.numeric(plmtest(FEFraud, c("time"), type = ("bp"))[2]) # Use
  ),
